...
class CKoretFuzzyHashing:
  ...
  def simplified(self, bytes, aggresive=False):
    ...
    buf = "".join(buf)
    return base64.b64encode(buf).strip(b"=")[:output_size]
  ...
  def _hash(self, bytes, aggresive=False):
    ...
    return base64.b64encode(ret).strip(b"=")[:output_size]
  ...
  def hash_file(self, filename, aggresive=False):
    ...
    if size > self.big_file_size:
      print()
      print("Warning! Support for big files (%d MB > %d MB) is broken!" % (
      size / 1024 / 1024, self.big_file_size / 1024 / 1024))
      fbytes = CFileStr(f)
    else:
      f.seek(0)
      fbytes = f.read()
